...
But notice how complicated its already become. What happens if you want to add another value? Another 10 values? An unspecified amount of values? Or what if you need to ensure the user's number meets certain criteria? Using our example above, one possible way we can define a function would be:
```go
